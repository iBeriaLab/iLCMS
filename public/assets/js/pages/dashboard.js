...
$(function() {
        ...
        var tooltip = d3.tip()
            ...
            .html(function (d) {
                return "<ul class='list-unstyled mb-5'>" +
                    "<li>" + "<div class='text-size-base mt-5 mb-5'><i class='icon-circle-left2 position-left'></i>" + "შეფასება" + "</div>" + "</li>" +
                    "<li>" + "მოწონება: &nbsp;" + "<span class='text-semibold pull-right'>" + d.value + "</span>" + "</li>" +
                    "<li>" + "დაწუნება: &nbsp; " + "<span class='text-semibold pull-right'>" + "$" + (d.value * 25).toFixed(2) + "</span>" + "</li>" +
                    "</ul>";
            });
            ...
            hourGroup.each(function(d) {
                heatMap
                    .on("mouseover", function (d, i) {
                        ...
                        d3.select(this.parentNode).select('.sales-count').text(function(d) { return d.values[i].value + " გადაწერა " + formatTime(d.values[i].date); })
                    })
                    .on("mouseout", function (d, i) {
                        d3.select(this).style('opacity', 1);
                        d3.select(this.parentNode).select('.sales-count').text(function (d, i) { return d3.sum(d.values, function(d) { return d.value; }) + " გადაწერა დღეს" })
                    })
            })
        ...
        var tooltip = d3.tip()
            ...
            .html(function (d) {
                return "<ul class='list-unstyled mb-5'>" +
                    ...
                    "<li>" + "დარიცხვა: &nbsp;" + "<span class='text-semibold pull-right'>" + d.alpha + "</span>" + "</li>" +
                    "<li>" + "შემოსავალი: &nbsp; " + "<span class='text-semibold pull-right'>" + "$" + (d.alpha * 25).toFixed(2) + "</span>" + "</li>" +
                    "</ul>";
            });
    ...
    // Initialize charts
    generateBarChart("#hours-available-bars", 24, 40, true, "elastic", 1200, 50, "#66BB6A", "hours");
    generateBarChart("#goal-bars", 24, 40, true, "elastic", 1200, 50, "#EC407A", "goal");
    ...
    function generateBarChart(element, barQty, height, animate, easing, duration, delay, color, tooltip) {
        ...
        if(tooltip == "hours") {
            tip.html(function (d, i) {
                return "<div class='text-center'>" +
                    ...
                    "<span class='text-size-small'>მოწონება</span>" +
        ...
        if(tooltip == "goal") {
            tip.html(function (d, i) {
                return "<div class='text-center'>" +
                    "<h6 class='no-margin'>" + d + "</h6>" +
                    "<span class='text-size-small'>Dislike</span>" +
                    "<div class='text-size-small'>" + i + ":00" + "</div>" +
                    "</div>"
            });
        }
        ...
        if(tooltip == "members") {
            tip.html(function (d, i) {
                return "<div class='text-center'>" +
                    ...
                    "<span class='text-size-small'>მომხმარებელი</span>" +
                    "<div class='text-size-small'>" + i + ":00" + "</div>" +
                    "</div>"
            });
        }
    ...
    // Initialize charts
    progressCounter('#hours-available-progress', 38, 2, "#66BB6A", like_p * 100, "icon-thumbs-up3 text-success-400", 'მოწონება', like_count)
    progressCounter('#goal-progress', 38, 2, "#F06292", dislike_p * 100, "icon-thumbs-down3 text-pink-400", 'Dislike', dislike_count)
    ...
    function progressCounter(element, radius, border, color, end, iconClass, textTitle, textAverage) {
        ...
        let numberText = d3.select(element)
            .append('h2')
